...
def menu_principal(vista):
    ...
    vista.barra_titulo = Label(
        vista.frame_ab,
        text="Analizador de Impacto Ambiental",
        background="#46dab7",
        foreground="black",
        font=("Arial", 14, "bold"),
    )
    ...
    entry_valor = ttk.Combobox(
        vista.frame_ab, textvariable=vista.var_categoria, width=15, state="readonly"
    )
    ...
    entry_descripcion = Entry(
        vista.frame_ab, textvariable=vista.var_descripcion, width=25
    )
    ...
    entry_valor = ttk.Combobox(
        vista.frame_ab, textvariable=vista.var_impacto, width=3, state="readonly"
    )
    ...
    boton_g = Button(
        vista.frame_ab,
        text="Guardar",
        command=lambda: vista.controlador.funcion_guardar(vista),
    )
    boton_g.grid(row=1, column=7, sticky="ew", padx=2)
    boton_d = Button(
        vista.frame_ab,
        text="Borrar",
        command=lambda: vista.controlador.funcion_borrar(vista.tree),
    )
    ...
    boton_t = Button(
        vista.frame_ab,
        text="Total",
        command=lambda: vista.controlador.funcion_total(vista.tree),
    )
    boton_t.grid(row=1, column=9, sticky="ew", padx=2)
    boton_p = Button(
        vista.frame_ab,
        text="Promedio",
        command=lambda: vista.controlador.funcion_promedio(vista.tree),
    )
    ...
    menu_archivo.add_command(
        label="Copiar",
        command=lambda: vista.controlador.copiar_fila(vista.tree, vista.root),
    )
    ...
    # con ejecución en vista tambien (Parametros)
    menu_archivo.add_command(
        label="Salir", command=lambda: vista.controlador.al_cerrar(vista.root)
    )
    menu_desplegable.add_cascade(
        label="Archivo", menu=menu_archivo
    )  # Hacemos que se añada esta cascada al menu 'menu desplegable'
    ...
    menu_ayuda.add_command(
        label="Instrucciones", command=lambda: vista.controlador.ver_instrucciones()
    )
    menu_ayuda.add_separator()  # Añadimos un separador por si en un futuro deseamos colocar mas botones
    menu_desplegable.add_cascade(
        label="Ayuda", menu=menu_ayuda
    )  # Hacemos que se añada esta cascada al menu 'menu desplegable'
    ...
    submenu_claro_oscuro.add_command(
        label="Oscuro", command=lambda: vista.modo_oscuro(vista)
    )
    submenu_claro_oscuro.add_separator()  # Añadimos un separador entre el botón 'Oscuro' y el botón 'Claro'
    submenu_claro_oscuro.add_command(
        label="Claro", command=lambda: vista.modo_claro(vista)
    )
...
def configurar_menu_modificar(vista):
    ...
    entry_valor = ttk.Combobox(
        vista.frame_modificacion,
        textvariable=vista.var_categoria,
        width=15,
        state="readonly",
    )
    entry_valor["values"] = ("Físico", "Biológico", "Socioeconómico")
    ...
    entry_descripcion = Entry(
        vista.frame_modificacion, textvariable=vista.var_descripcion, width=25
    )
    ...
    entry_valor = ttk.Combobox(
        vista.frame_modificacion,
        textvariable=vista.var_impacto,
        width=3,
        state="readonly",
    )
    entry_valor["values"] = (-1, 0, 1)
    ...
    boton_refresh = Button(
        ...
        text="Actualizar",
        width=15,
        command=lambda: vista.controlador.funcion_modificar_variables(vista),
    )
    ...
    boton_save = Button(
        ...
        text="Volver",
        width=15,
        command=lambda: vista.funcion_volver_menu_principal(),
    )
...
def configurar_menu_consulta(vista):
    ...
    etiqueta_nombre_search1 = Label(
        vista.frame_consulta, text="Busque por Id, Categoría, Descripción o Impacto"
    )
    ...
    entry_id_search = Entry(
        vista.frame_consulta, textvariable=vista.var_busqueda, width=20
    )
    entry_id_search.grid(row=1, column=0, columnspan=2)

    # Creamos y colocamos los botones 'Buscar' y 'Volver'
    boton_busqueda = Button(
        vista.frame_consulta,
        text="Buscar",
        width=25,
        command=lambda: vista.controlador.funcion_busqueda(vista),
    )
    boton_busqueda.grid(row=0, column=2, columnspan=2, sticky=W + E)
    boton_menu_principal = Button(
        vista.frame_consulta,
        text="Volver",
        width=25,
        command=vista.funcion_volver_menu_principal,
    )
